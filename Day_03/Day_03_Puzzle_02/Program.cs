...
namespace Day_03_Puzzle_02
{
    internal static class Program
    {
        ...
        private static int _runningTotal = 0;
        ...
        static void Main(string[] args)
        {
            ...
            // The first row only contains numbers.
            for (int rowNumber = 1; rowNumber < calibrationData.Length; rowNumber++)
            {
                _runningTotal += ProcessEngineSchematicRow(calibrationData, rowNumber);
            }
        ...
        private static int ProcessEngineSchematicRow(string[] calibrationData, int rowNumber)
        {
            int subtotal = 0;
            string currentRow = calibrationData[rowNumber];

            for (int currentRowIndex = 0; currentRowIndex < currentRow.Length; currentRowIndex++)
            {
                // we only care about the non-number/'.' characters.
                if (Equals(currentRow[currentRowIndex], '*'))
                {
                    subtotal += GetGearRatio(calibrationData, rowNumber, currentRowIndex);
                }
            }
        ...
        private static int GetGearRatio(string[] calibrationData, int rowNumber, int currentRowIndex)
        {
            List<int> numbers = new List<int>();
            ReadOnlySpan<char> previousRow = new ReadOnlySpan<char>(calibrationData[rowNumber - 1].ToCharArray());
            ReadOnlySpan<char> currentRow = new ReadOnlySpan<char>(calibrationData[rowNumber].ToCharArray());
            ReadOnlySpan<char> nextRow = new ReadOnlySpan<char>(calibrationData[rowNumber + 1].ToCharArray());
            ...
            // check previous row
            for (int indexOffset = -1; indexOffset < 2; indexOffset++)
            {
                rootIndex = currentRowIndex + indexOffset;
                gearPartNumber = GetRowNumber(ref previousRow, rootIndex, out rightDigitCount);
                if (gearPartNumber > 0)
                {
                    numbers.Add(gearPartNumber);
                }

                // we don't need to recheck these if they were already parsed.
                indexOffset += rightDigitCount;
            }
            // early out
            if (numbers.Count > 2)
            {
            ...
            {
                rootIndex = currentRowIndex + indexOffset;
                gearPartNumber = GetRowNumber(ref nextRow, rootIndex, out rightDigitCount);
                if (gearPartNumber > 0)
                {
                    numbers.Add(gearPartNumber);
                }
            ...
            // check current row
            // check to the left (current character cannot be a digit
            rootIndex = currentRowIndex - 1;
            gearPartNumber = GetRowNumber(ref currentRow, rootIndex, out _);
            if (gearPartNumber > 0)
            {
                numbers.Add(gearPartNumber);
            }
            ...
            // only gears with exactly 2 numbers count
            if (numbers.Count != 2)
            {
                return 0;
            }
        ...
        private static int GetRowNumber(ref ReadOnlySpan<char> testRow, int rootIndex, out int rightDigitCount)
        {
            char currentChar;
            ...
            currentChar = testRow[rootIndex];
            // if the initial digit isn't a number, move on.
            if (!char.IsDigit(currentChar))
            {
                return 0;
            }

            currentCharIndex = rootIndex;
            leftDigitCount = 0;
            rightDigitCount = 0;
            // check initial character and to the left
            nextCharIndex = currentCharIndex - 1;
            while (currentCharIndex > -1 && char.IsDigit(currentChar))
            {
                leftDigitCount++;
                currentCharIndex--;
                if (nextCharIndex > -1)
                {
                    currentChar = testRow[nextCharIndex];
                }
                nextCharIndex--;
            }

            // check to the right
            currentChar = testRow[rootIndex + 1];
            currentCharIndex = rootIndex + 1;
            nextCharIndex = currentCharIndex + 1;
            while (currentCharIndex < testRow.Length && char.IsDigit(currentChar))
            {
                rightDigitCount++;
                currentCharIndex++;
                if (nextCharIndex < testRow.Length)
                {
                    currentChar = testRow[nextCharIndex];
                }
                nextCharIndex++;
            ...
            int totalDigitCount = leftDigitCount + rightDigitCount;
            if (totalDigitCount > 0)
            {
                number = int.Parse(testRow.Slice(rootIndex - (leftDigitCount - 1), totalDigitCount));
            }
