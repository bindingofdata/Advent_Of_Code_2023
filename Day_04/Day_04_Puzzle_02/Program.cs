...
namespace Day_04_Puzzle_02
{
    internal static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Card[] scratcherCards = ParseCards(scratcherRounds);
            CalculateCardInstances(scratcherCards);
            foreach (Card card in scratcherCards)
            {
                _runningTotal += card.CardInstances;
            }
        ...
        private static void CalculateCardInstances(Card[] scratcherCards)
        {
            foreach (Card scratcherCard in scratcherCards)
            {
                scratcherCard.CardInstances++;
                int nextCardIndex = scratcherCard.CardIndex + 1;
                for (int nextCardOffset = 0; nextCardOffset < scratcherCard.MatchCount; nextCardOffset++)
                {
                    scratcherCards[nextCardIndex + nextCardOffset].CardInstances += scratcherCard.CardInstances;
                }
            }
        }

        private static void FlagInstances(Card scratcherCard, int remainingTestValues, Card[] scratcherCards)
        {
            int nextCardIndex = scratcherCard.CardIndex + 1;
            for (int offset = 0; offset < scratcherCard.MatchCount; offset++)
            {
                if (remainingTestValues > 0)
                {
                    FlagInstances(scratcherCards[nextCardIndex + offset], remainingTestValues - 1, scratcherCards);
                }
            }
        ...
        private static Card[] ParseCards((string CardInfo, string UserNumbers)[] scratcherRounds)
        {
            List<Card> scratcherCards = new List<Card>();
            for (int index = 0; index < scratcherRounds.Length; index++)
            {
                (string CardInfo, string UserNumbers) scratcherCardString = scratcherRounds[index];
                Card card = new Card()
                {
                    CardIndex = index,
                    MatchCount = CountMatches(scratcherCardString.CardInfo, scratcherCardString.UserNumbers),
                };
                scratcherCards.Add(card);
            }
            return scratcherCards.ToArray();
        }

        private static int CountMatches(string cardInfo, string userNumbersString)
        {
            int matches = 0;
            int[] sortedCardNumbers = GetCardNumbers(cardInfo, true).ToArray();

            foreach (int cardNumber in GetCardNumbers(userNumbersString, false))
            {
        ...
        private static IEnumerable<int> GetCardNumbers(string sourceString, bool sort)
        {
    ...
    internal class Card
    {
        ...
        public int CardInstances { get; set; }
    }
}
