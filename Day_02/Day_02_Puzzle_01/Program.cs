...
namespace Puzzle_01
{
    internal static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            foreach (string game in calibrationData)
            {
                if (ParseGameData(game, out int gameNumber))
                {
        ...
        private static bool ParseGameData(string data, out int gameNumber)
        {
            // all lines start with "Game "
            string[] gameNumberAndData = data.Split(':');
            gameNumber = GetGameNumber(gameNumberAndData[0]);
            return ValidateDicePulls(gameNumberAndData[1]);
        }

        private static bool ValidateDicePulls(string dicePullData)
        {
            string[][] pulls = dicePullData
                .Split(";")
                    .Select(pullGroup => pullGroup
                        .Split(",")
                            .Select(pull => pull.Trim())
                            .ToArray())
                    .ToArray();

            foreach (string[] pullGroup in pulls)
            {
                for (int i = 0; i < pullGroup.Length; i++)
                {
                    // number, color string
                    string[] pull = pullGroup[i].Split(" ");
                    int maxAllowed = 0;
                    switch (pull[1])
                    {
                        case "red":
                    ...
                    if (int.Parse(pull[0]) > maxAllowed)
                    {
                        return false;
                    }
                }
        ...
        private static int GetGameNumber(string gameNumber)
        {
            return int.Parse(gameNumber.Split(" ")[1]);
        }
