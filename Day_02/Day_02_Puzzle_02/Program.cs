...
namespace Day_02_Puzzle_01
{
    internal static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                runningTotal += GetGamePower(game);
            }
        ...
        private static int GetGamePower(string data)
        {
            // all lines start with "Game "
            string gameData = data.Split(":")[1].TrimStart();
            return GetProductOfSmallestValues(gameData);
        }

        private static int GetProductOfSmallestValues(string gameData)
        {
            string[][] pulls = gameData
                .Split(";")
            ...
            foreach (string[] pullGroup in pulls)
            {
                for (int i = 0; i < pullGroup.Length; i++)
                {
                    ...
                    int count = int.Parse(pull[0]);

                    switch (pull[1])
                    {
                        case "red":
                            if (count > requiredRed)
                            {
                                requiredRed = count;
                            }
                            break;
                        case "green":
                            if (count > requiredGreen)
                            {
                                requiredGreen = count;
                            }
                            break;
                        case "blue":
                            if (count > requiredBlue)
                            {
                                requiredBlue = count;
                            }
                            break;
